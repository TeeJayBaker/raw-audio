...
class MeanFlow(RectifiedFlow):
    ...
    def u_and_dudt(
        ...
        """(u, du/dt, u-head audio, u-head channelised spec) at (x_t, t, h). The trainer supplies the
        v-head instantaneous velocity as ``tangent``; du/dt is detached in both modes. audio/spec are
        the u-head prediction, returned for the waveform / complex-STFT aux losses."""
        if mode not in {"dde", "jvp"}:
            raise ValueError("flow.mf.dudt must be 'dde' or 'jvp'")
        kwargs = dict(cond=cond, omega=omega, t_lo=t_lo, t_hi=t_hi, length=length)

        def u_fn(z, t_, h_):
            pred, _aux, audio, spec = self._predict(model, z, with_aux=False, t=t_, h=h_, **kwargs)
            return self.target_to_v(pred, z, t_), audio, spec
        ...
        if mode == "jvp":
            ...
            try:
                (u, audio, spec), (dudt, _, _) = torch.func.jvp(
                    u_fn,
                    (x_t, t, h),
            ...
            return u, dudt.detach(), audio, spec

        u, audio, spec = u_fn(x_t, t, h)
        ...
        def u_probe(z, t_, h_):
            pred = self._predict(
                model, z, with_aux=False, t=t_, h=h_, cond=cond_f, omega=omega_f, t_lo=t_lo_f, t_hi=t_hi_f, length=length
            )[0]
            return self.target_to_v(pred, z, t_)
        ...
        return u, dudt.to(u.dtype), audio, spec
    ...
    def generate(
        ...
        for i in range(steps):
            ...
            dt = grid[i + 1] - grid[i]
            h = dt.expand(batch)
            x_pred, _aux, _audio, spec = self._predict(
                model, x, with_aux=False, t=t, h=h, cond=cond, omega=omega, t_lo=t_lo, t_hi=t_hi, length=length
            )
